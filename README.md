...
Analisar algoritmos e métricas relacionados ao aprendizado supervisionado do tipo regressão.

Após verificação dos dados, serão definidos alguns métodos e métricas referentes à problemas de regressão. A ideia não é se ater a detalhes de funcionamento de cada método, mas sim entender quais métricas podem ser consideradas nessa situação e propor uma forma de explicar a influência de cada atributo nos resultados.
...
Mais informações em: [Link](https://scikit-learn.org/stable/modules/generated/sklearn.datasets.load_diabetes.html#sklearn.datasets.load_diabetes)
...
Só será verificado se existe alguma instância com valor NA. Caso positivo, esta será excluída. E também se existem instâncias duplicadas.
...
## Dividir base de dados para treinamento e validação
...
```python
# Visualizar Variância Explicada para cada método
...
```python
# Visualizar MSE para cada método
...
```python
# Visualizar R2 para cada método
...
## Influência dos atributos nos resultados
...
Mais sobre: [Link](https://towardsdatascience.com/explain-your-model-with-the-shap-values-bc36aac4de3d)
